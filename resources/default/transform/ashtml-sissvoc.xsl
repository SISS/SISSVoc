...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fn="http://www.w3.org/2005/xpath-functions">
	...
	<xsl:template match="format/item" mode="metalink">
		<link rel="alternate" href="{$resourcePath}?uri={@href}" type="{format/label}"/>
	</xsl:template>
	...
	<xsl:template match="result" mode="script">
		...
		<xsl:if test="$showMap = 'true'">
			<script src="{$_resourceRoot}openlayers/OpenLayers.js"/>
			<script src="{$_resourceRoot}openlayers/OpenStreetMap.js"/>
			<script src="{$_resourceRoot}openlayers/proj4js-combined.js"/>
			<script src="http://spatialreference.org/ref/epsg/27700/proj4js/"/>
			<script src="http://spatialreference.org/ref/epsg/4326/proj4js/"/>
		</xsl:if>
		<script type="text/javascript" src="{$_resourceRoot}scripts/jquery.min.js"/>
		<script type="text/javascript" src="{$_resourceRoot}scripts/jquery-ui.min.js"/>
		<script type="text/javascript" src="{$_resourceRoot}scripts/jquery.sparkline.js"/>
		<script type="text/javascript" src="{$_resourceRoot}scripts/codemirror/codemirror_min.js"/>
            ...
            <xsl:if test="$showMap = 'true'">
				...
				<xsl:variable name="windowUri">
					<xsl:call-template name="substituteParam">
						...
						<xsl:with-param name="param">min-<xsl:value-of select="$longParam"/>
						</xsl:with-param>
						<xsl:with-param name="uri">
							<xsl:call-template name="substituteParam">
								<xsl:with-param name="value" select="''"/>
								<xsl:with-param name="param">max-<xsl:value-of select="$longParam"/>
								</xsl:with-param>
								<xsl:with-param name="uri">
									<xsl:call-template name="substituteParam">
										<xsl:with-param name="value" select="''"/>
										<xsl:with-param name="param">min-<xsl:value-of select="$latParam"/>
										</xsl:with-param>
										<xsl:with-param name="uri">
											<xsl:call-template name="substituteParam">
												<xsl:with-param name="value" select="''"/>
	...
	<xsl:template match="viewingResult | selectionResult" mode="footer">
		<div>
			<xsl:attribute name="class"><xsl:variable name="selquery"><xsl:value-of select="self::viewingResult[../selectionResult/query/value]"/></xsl:variable><xsl:text> query col</xsl:text><xsl:choose><xsl:when test="$selquery != ''">2</xsl:when><xsl:otherwise>1</xsl:otherwise></xsl:choose></xsl:attribute>
			<h2>
				<xsl:choose>
			...
			<xsl:variable name="endPointToUse">
				<xsl:choose>
					...
					<xsl:otherwise>
						<xsl:value-of select="$visibleSparqlEndpoint"/>
					</xsl:otherwise>
				</xsl:choose>
			</xsl:variable>
			<xsl:variable name="formToUse">
				<xsl:choose>
					...
					<xsl:otherwise>
						<xsl:value-of select="$visibleSparqlForm"/>
					</xsl:otherwise>
				</xsl:choose>
			</xsl:variable>
			<p>
	...
	<xsl:template match="query" mode="footer">
		<textarea name="query" wrap="off">
			<xsl:attribute name="data-skip-lines"><xsl:apply-templates select="." mode="skipLines"/></xsl:attribute>
	...
	<xsl:template match="result" mode="map">
		...
		<xsl:choose>
			<xsl:when test="$showMap = 'true'">
				...
				<xsl:variable name="searchMap" select="//*[*[name(.) = $long] and *[name(.) = $lat]]"/>
				<section class="map">
					<h1>Map</h1>
					<xsl:call-template name="createInfo">
...
-->
			...
			<tr>
				<xsl:choose>
					<xsl:when test="$paramName = concat('min-', $long)">
						...
						<td class="filter">
							<a title="remove filter">
								<xsl:attribute name="href"><xsl:call-template name="clearPosition"><xsl:with-param name="uri"><xsl:apply-templates select="/result" mode="searchURI"/></xsl:with-param></xsl:call-template></xsl:attribute>
					...
					<xsl:otherwise>
						...
						<td class="filter">
							<a title="remove filter">
								<xsl:attribute name="href"><xsl:call-template name="substituteParam"><xsl:with-param name="uri"><xsl:apply-templates select="/result" mode="searchURI"/></xsl:with-param><xsl:with-param name="param" select="$paramName"/><xsl:with-param name="value" select="''"/></xsl:call-template></xsl:attribute>
								<img src="{$activeImageBase}/Back.png" alt="remove filter"/>
							</a>
						</td>
					</xsl:otherwise>
			...
			<xsl:if test="$properties != ''">
				<p class="reset">
					<a title="unstar all properties">
						<xsl:attribute name="href"><xsl:call-template name="substituteParam"><xsl:with-param name="uri" select="/result/@href"/><xsl:with-param name="param" select="'_properties'"/><xsl:with-param name="value" select="''"/></xsl:call-template></xsl:attribute>
	...
	<xsl:template match="result" mode="selectedProperties">
		...
		<li class="selected">
			<a rel="nofollow" title="remove this property">
				<xsl:attribute name="href"><xsl:call-template name="substituteParam"><xsl:with-param name="uri" select="@href"/><xsl:with-param name="param" select="'_properties'"/><xsl:with-param name="value"><xsl:if test="$previousProperties != ''"><xsl:value-of select="$previousProperties"/><xsl:text>,</xsl:text></xsl:if><xsl:value-of select="substring-after($properties, ',')"/></xsl:with-param></xsl:call-template></xsl:attribute>
				<img src="{$activeImageBase}/Star.png" alt="unstar this property"/>
			</a>
			<xsl:text> </xsl:text>
			<xsl:choose>
		...
		<xsl:if test="not(contains(concat(',', $properties, ','), concat(',', $name, ',')))">
			<li>
				<a rel="nofollow" title="always include this property">
					<xsl:attribute name="href"><xsl:call-template name="substituteParam"><xsl:with-param name="uri" select="/result/@href"/><xsl:with-param name="param" select="'_properties'"/><xsl:with-param name="value"><xsl:if test="$properties != ''"><xsl:value-of select="$properties"/><xsl:text>,</xsl:text></xsl:if><xsl:value-of select="$name"/></xsl:with-param></xsl:call-template></xsl:attribute>
					<img src="{$inactiveImageBase}/Star.png" alt="star this property"/>
					<xsl:text> </xsl:text>
					<xsl:apply-templates select="." mode="contextLabel"/>
				</a>
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				<a rel="nofollow" title="view {$size} items per page">
					<xsl:attribute name="href"><xsl:call-template name="substituteParam"><xsl:with-param name="uri"><xsl:apply-templates select="/result" mode="searchURI"/></xsl:with-param><xsl:with-param name="param" select="'_pageSize'"/><xsl:with-param name="value" select="$size"/></xsl:call-template></xsl:attribute>
					<xsl:value-of select="$size"/>
				</a>
			</xsl:otherwise>
		</xsl:choose>
			...
			<xsl:if test="$current != ''">
				<p class="reset">
					<a title="remove sorting">
						<xsl:attribute name="href"><xsl:call-template name="substituteParam"><xsl:with-param name="uri" select="$baseURI"/><xsl:with-param name="param" select="'_sort'"/><xsl:with-param name="value" select="''"/></xsl:call-template></xsl:attribute>
						<img src="{$activeImageBase}/Back.png" alt="reset"/>
					</a>
				</p>
			</xsl:if>
							...
							<xsl:choose>
								...
								<xsl:when test="starts-with($orderBy, 'desc')">
									<a rel="nofollow" title="sort in ascending order">
										<xsl:attribute name="href"><xsl:call-template name="substituteParam"><xsl:with-param name="uri" select="$baseURI"/><xsl:with-param name="param" select="'_orderBy'"/><xsl:with-param name="value" select="substring-after($orderBy, 'desc')"/></xsl:call-template></xsl:attribute>
										<img src="{$activeImageBase}/Arrow3 Down.png" alt="sort in ascending order"/>
									</a>
								...
								<xsl:when test="starts-with($orderBy, 'asc')">
									<a rel="nofollow" title="sort in descending order">
										<xsl:attribute name="href"><xsl:call-template name="substituteParam"><xsl:with-param name="uri" select="$baseURI"/><xsl:with-param name="param" select="'_orderBy'"/><xsl:with-param name="value" select="concat('desc', substring-after($orderBy, 'asc'))"/></xsl:call-template></xsl:attribute>
										<img src="{$activeImageBase}/Arrow3 Up.png" alt="sort in descending order"/>
									</a>
									<xsl:value-of select="$description"/>
								</xsl:when>
								<xsl:otherwise>
									<a rel="nofollow" title="sort in descending order">
										<xsl:attribute name="href"><xsl:call-template name="substituteParam"><xsl:with-param name="uri" select="$baseURI"/><xsl:with-param name="param" select="'_orderBy'"/><xsl:with-param name="value" select="concat('desc', $orderBy)"/></xsl:call-template></xsl:attribute>
										<img src="{$activeImageBase}/Arrow3 Up.png" alt="sort in descending order"/>
									</a>
									<xsl:value-of select="$description"/>
								</xsl:otherwise>
	...
	<xsl:template match="result" mode="selectedSorts">
		...
		<li class="selected">
			<a rel="nofollow" title="remove this sort">
				<xsl:attribute name="href"><xsl:call-template name="substituteParam"><xsl:with-param name="uri" select="$uri"/><xsl:with-param name="param" select="'_sort'"/><xsl:with-param name="value"><xsl:if test="$previousSorts != ''"><xsl:value-of select="$previousSorts"/><xsl:text>,</xsl:text></xsl:if><xsl:value-of select="substring-after($sorts, ',')"/></xsl:with-param></xsl:call-template></xsl:attribute>
				<img src="{$activeImageBase}/Cancel.png" alt="remove this sort"/>
			</a>
			<a rel="nofollow">
				<xsl:attribute name="href"><xsl:call-template name="substituteParam"><xsl:with-param name="uri" select="$uri"/><xsl:with-param name="param" select="'_sort'"/><xsl:with-param name="value"><xsl:if test="$previousSorts != ''"><xsl:value-of select="$previousSorts"/><xsl:text>,</xsl:text></xsl:if><xsl:choose><xsl:when test="starts-with($sort, '-')"><xsl:value-of select="substring($sort, 2)"/></xsl:when><xsl:otherwise><xsl:value-of select="concat('-', $sort)"/></xsl:otherwise></xsl:choose></xsl:with-param></xsl:call-template></xsl:attribute>
				<xsl:attribute name="title"><xsl:choose><xsl:when test="starts-with($sort, '-')">sort in ascending order</xsl:when><xsl:otherwise>sort in descending order</xsl:otherwise></xsl:choose></xsl:attribute>
				<xsl:choose>
					<xsl:when test="starts-with($sort, '-')">
						<img src="{$activeImageBase}/Arrow3 Down.png" alt="sort in ascending order"/>
					</xsl:when>
	...
	<xsl:template match="*" mode="sort">
		...
		<xsl:if test="not(contains(concat(',', $current, ','), concat(',', $name, ',')) or contains(concat(',', $current, ','), concat(',-', $name, ',')))">
			...
			<li>
				...
				<a rel="nofollow" title="sort in descending order">
					<xsl:attribute name="href"><xsl:call-template name="substituteParam"><xsl:with-param name="uri" select="$uri"/><xsl:with-param name="param" select="'_sort'"/><xsl:with-param name="value"><xsl:if test="$current != ''"><xsl:value-of select="$current"/><xsl:text>,</xsl:text></xsl:if><xsl:text>-</xsl:text><xsl:value-of select="$name"/></xsl:with-param></xsl:call-template></xsl:attribute>
					<img src="{$inactiveImageBase}/Arrow3 Down.png" alt="sort in descending order"/>
				</a>
				<xsl:text> </xsl:text>
				<a rel="nofollow" href="{$ascending}" title="sort in ascending order">
	...
	<xsl:template match="*" mode="paramName">
		...
		<xsl:choose>
			<xsl:when test="item">
				<xsl:apply-templates select="item[1]" mode="paramName"/>
			</xsl:when>
			<xsl:when test="$bestLabelParam != ''">
				<xsl:apply-templates select="*[name() = $bestLabelParam]" mode="paramHierarchy"/>
			</xsl:when>
			<xsl:otherwise>
				<xsl:apply-templates select="." mode="paramHierarchy"/>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
				...
				<xsl:choose>
					...
					<xsl:when test="$hasNonLabelProperties = 'true' or item">
						<td class="value nested">
							<xsl:attribute name="colspan"><xsl:choose><xsl:when test="$showMap = 'true'">3</xsl:when><xsl:otherwise>2</xsl:otherwise></xsl:choose></xsl:attribute>
							<xsl:apply-templates select="." mode="display">
...
-->
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				<a rel="nofollow" title="add this property">
					<xsl:attribute name="href"><xsl:call-template name="substituteParam"><xsl:with-param name="uri" select="/result/@href"/><xsl:with-param name="param" select="'_properties'"/><xsl:with-param name="value"><xsl:if test="$properties != ''"><xsl:value-of select="$properties"/><xsl:text>,</xsl:text></xsl:if><xsl:value-of select="$paramName"/></xsl:with-param></xsl:call-template></xsl:attribute>
					<img src="{$inactiveImageBase}/Star.png" alt="star this property"/>
				</a>
			</xsl:otherwise>
		</xsl:choose>
		...
		<xsl:choose>
			...
			<xsl:when test="$datatype = 'integer' or $datatype = 'decimal' or $datatype = 'float' or $datatype = 'int' or $datatype = 'date' or $datatype = 'dateTime' or $datatype = 'time'">
				...
				<xsl:choose>
					<xsl:when test="$max = $value">
						<a rel="nofollow" title="remove maximum value filter">
							<xsl:attribute name="href"><xsl:call-template name="substituteParam"><xsl:with-param name="uri"><xsl:apply-templates select="/result" mode="searchURI"/></xsl:with-param><xsl:with-param name="param" select="concat('max-', $paramName)"/><xsl:with-param name="value" select="''"/></xsl:call-template></xsl:attribute>
							<img src="{$activeImageBase}/Back.png" alt="remove maximum value filter"/>
						</a>
					</xsl:when>
					<xsl:otherwise>
						<a rel="nofollow" title="filter to values less than {$value}">
							<xsl:attribute name="href"><xsl:call-template name="substituteParam"><xsl:with-param name="uri"><xsl:apply-templates select="/result" mode="searchURI"/></xsl:with-param><xsl:with-param name="param" select="concat('max-', $paramName)"/><xsl:with-param name="value" select="$value"/></xsl:call-template></xsl:attribute>
							<xsl:choose>
								<xsl:when test="$max != ''">
				...
				<a rel="nofollow" title="more like this">
					<xsl:attribute name="href"><xsl:call-template name="substituteParam"><xsl:with-param name="uri"><xsl:apply-templates select="/result" mode="searchURI"/></xsl:with-param><xsl:with-param name="param" select="$paramName"/><xsl:with-param name="value" select="$value"/></xsl:call-template></xsl:attribute>
				...
				<xsl:choose>
					<xsl:when test="$min = $value">
						<a rel="nofollow" title="remove minimum value filter">
							<xsl:attribute name="href"><xsl:call-template name="substituteParam"><xsl:with-param name="uri"><xsl:apply-templates select="/result" mode="searchURI"/></xsl:with-param><xsl:with-param name="param" select="concat('min-', $paramName)"/><xsl:with-param name="value" select="''"/></xsl:call-template></xsl:attribute>
							<img src="{$activeImageBase}/Back.png" alt="remove minimum value filter"/>
						</a>
					</xsl:when>
					<xsl:otherwise>
						<a rel="nofollow" title="more than {$value}">
							<xsl:attribute name="href"><xsl:call-template name="substituteParam"><xsl:with-param name="uri"><xsl:apply-templates select="/result" mode="searchURI"/></xsl:with-param><xsl:with-param name="param" select="concat('min-', $paramName)"/><xsl:with-param name="value" select="$value"/></xsl:call-template></xsl:attribute>
							<xsl:choose>
								<xsl:when test="$min != ''">
			...
			<xsl:otherwise>
				<a rel="nofollow" title="more like this">
					<xsl:attribute name="href"><xsl:call-template name="substituteParam"><xsl:with-param name="uri"><xsl:apply-templates select="/result" mode="searchURI"/></xsl:with-param><xsl:with-param name="param" select="$paramName"/><xsl:with-param name="value" select="$label"/></xsl:call-template></xsl:attribute>
					<img src="{$inactiveImageBase}/Search.png" alt="more like this"/>
				</a>
			</xsl:otherwise>
		</xsl:choose>
	...
	<xsl:template match="@href" mode="wicep-uri">
